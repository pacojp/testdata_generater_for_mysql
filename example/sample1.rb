...
#
# 次に各列の処理を設定します
# データはハッシュにて設定します。設定したいカラムの名前をkeyにて設定しvalueはProcインスタンスにて設定します。Procのイニシャライザのブロック引数にloopsで設定した値が引き渡されます(上記サンプルのso_somethingのところの値がハッシュにて引き渡されます)。
# 基本的にvalueは自動的にエスケープされシングルクォーテーションで囲まれます。「NOW()」等関数を指定したい場合は"NOW()".to_funcと指定すると値にエスケープ及びシングルクォーテーションでの囲みがかからなくなります
#
procs = {
